...
void frmMain::initForm()
{
    ...
    ui->label_29->setText(companyName);
    ...
    //摄像机地址加载
    QStringList urls;
    urls <<configIniWrite->value("live/vc_1").toString();
    urls <<configIniWrite->value("live/vc_2").toString();
    urls <<configIniWrite->value("live/vc_3").toString();
    urls <<configIniWrite->value("live/vc_4").toString();
    ui->widget_4->setUrls(urls);
    ui->widget_5->setUrls(urls);
    ui->widget_5->setRightclickSt(false);
    ui->lineEdit_9->setText(urls.at(0));
    ui->lineEdit_17->setText(urls.at(1));
    ui->lineEdit_18->setText(urls.at(2));
    ui->lineEdit_20->setText(urls.at(3));
...
{
    QString data = tableview_2->updateData();
    if(!data.isNull()) {
        ui->groupBox_11->setTitle(data);
        QUIWidget::showMessageBoxInfo("更新完成!", 3);
    } else {
        QUIWidget::showMessageBoxInfo("更新失败!", 3);
    }
...
{
    configIniWrite->setValue("live/vc_1",ui->lineEdit_9->text());
    configIniWrite->setValue("live/vc_2",ui->lineEdit_17->text());
    configIniWrite->setValue("live/vc_3",ui->lineEdit_18->text());
    configIniWrite->setValue("live/vc_4",ui->lineEdit_20->text());
    QStringList urls;
    urls <<configIniWrite->value("live/vc_1").toString();
    urls <<configIniWrite->value("live/vc_2").toString();
    urls <<configIniWrite->value("live/vc_3").toString();
...
void frmMain::on_pushButton_49_clicked()
{
    int result = tableview_2->conditionQuery();
    if(result) {
        QUIHelper::showMessageBoxInfo("查询成功!", 3);
    } else {
        QUIHelper::showMessageBoxError("查询失败!", 3);
